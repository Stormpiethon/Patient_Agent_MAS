...
async def llm_lab_analysis(
    ...
    return {
        "health_concerns": response.choices[0].message.content,
...
async def llm_cost_analysis(
    ...
    response = await client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You summarize healthcare billing information.",
            },
            {
                "role": "user",
                "content": prompt,
    ...
    return {
        "cost_summary": response.choices[0].message.content,
        "prompt_tokens": response.usage.prompt_tokens,
        "completion_tokens": response.usage.completion_tokens,
        "total_tokens": response.usage.total_tokens,
    }
